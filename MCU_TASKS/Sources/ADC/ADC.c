...
ADC_INTPUT_t adc_intput	[kADC_CHANNELS];
...
void ADC_init (void)
{
	...
	ADC_channel_init (&adc_intput[kADC_GPIO_IN1   ], ADC_GPIO_IN1   ,     0,     110,     10,    false);
	ADC_channel_init (&adc_intput[kADC_GPIO_IN2   ], ADC_GPIO_IN2   ,     0,     110,     10,    false);
	ADC_channel_init (&adc_intput[kADC_GPIO_IN3   ], ADC_GPIO_IN3   ,     0,     110,     10,    true );
	ADC_channel_init (&adc_intput[kADC_GPIO_IN4   ], ADC_GPIO_IN4   ,     0,     110,     10,    false);
	ADC_channel_init (&adc_intput[kADC_GPIO_IN5   ], ADC_GPIO_IN5   ,     0,     110,     10,    false);
	ADC_channel_init (&adc_intput[kADC_GPIO_IN6   ], ADC_GPIO_IN6   ,     0,     110,     10,    false);
	ADC_channel_init (&adc_intput[kADC_GPIO_IN7   ], ADC_GPIO_IN7   ,     0,     110,     10,    false);
...
void ADC_sample_input (KADC_CHANNELS_t channel)
{
	...
	ADC16_DRV_ConfigConvChn (ADC16_INSTANCE, ADC16_CHN_GROUP_0, &adc_intput[channel].chnConfig);				// trigger the conversion
	...
	if (adc_intput[channel].chnConfig.diffConvEnable) {
		// in case of differential pair sample positive input and calculate the value of negative pad
		adc_intput[channel].chnConfig.diffConvEnable = false;
		ADC16_DRV_ConfigConvChn (ADC16_INSTANCE, ADC16_CHN_GROUP_0, &adc_intput[channel].chnConfig);			// trigger the conversion
		ADC16_DRV_WaitConvDone  (ADC16_INSTANCE, ADC16_CHN_GROUP_0);											// Wait for the conversion to be done
		...
		if (sample1 < 0)
			sample1 = 0;
		adc_intput[channel].chnConfig.diffConvEnable = true;
	}
	...
	adc_intput[channel].sample = (uint32_t) sample2;

	ADC_convert_sample_to_value (&adc_intput[channel]);
}	
...
{
	assert (channel < kADC_CHANNELS);
	return adc_intput[channel].value;
}
